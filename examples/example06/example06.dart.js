...
$$.LinkedHashMapImplementation = {"":
 ...
 LinkedHashMapImplementation$0: function() {
  this._lib0_map = $.HashMapImplementation$();
...
$$.OnesanRobot = {"":
 ["_row", "_col", "_map", "_route"],
 "super": "Object",
 _init$0: function() {
  this._route = 0;
  this._map = $.ListImplementation_List(this._row);
  for (var t1 = this._col, i = 0; $.ltB(i, $.get$length(this._map)); ++i) {
    $.indexSet(this._map, i, $.ListImplementation_List(t1));
    for (var j = 0; $.ltB(j, $.get$length($.index(this._map, i))); ++j)
      $.indexSet($.index(this._map, i), j, false);
  }
},
 start$0: function() {
  $.window().alert$1($.S($.sub(this._row, 1)) + 'x' + $.S($.sub(this._col, 1)) + '\u30c7\u30b1\u30a4\u30b5\u30f3\u3000\u30b9\u30eb\u30e8\uff01');
  this._update$3(0, 0, this._copy$1(this._map));
  $.query('#route').set$text('\u30b1\u30a4\u30ed\u30bf\u30f3\u30b5\u30af\u3000\u30aa\u30ef\u30ea\u30de\u30b7\u30bf\u3000\u30b1\u30a4\u30ed\u30b9\u30a6\u30cf\u3000' + $.S(this._route) + '\u3000\u30c7\u30b7\u30bf\u3002');
},
 _copy$1: function(m) {
  if (typeof m !== 'string' && (typeof m !== 'object' || m === null || m.constructor !== Array && !m.is$JavaScriptIndexingBehavior()))
    return this._copy$1$bailout(1, m, 0, 0, 0, 0, 0, 0);
  var retMap = $.ListImplementation_List(this._row);
  for (var t1 = this._col, i = 0; i < retMap.length; ++i) {
    var t2 = $.ListImplementation_List(t1);
    if (i < 0 || i >= retMap.length)
      throw $.ioore(i);
    retMap[i] = t2;
    var j = 0;
    while (true) {
 ...
 _copy$1$bailout: function(state, env0, env1, env2, env3, env4, env5, env6) {
  switch (state) {
    case 1:
      var m = env0;
      break;
 ...
 _update$3: function(posX, posY, map) {
  var copyMap = this._copy$1(map);
  if (typeof copyMap !== 'string' && (typeof copyMap !== 'object' || copyMap === null || copyMap.constructor !== Array && !copyMap.is$JavaScriptIndexingBehavior()))
    return this._update$3$bailout(1, posX, posY, copyMap, 0);
  if (!(posX < 0)) {
    var t1 = this._col;
    if (typeof t1 !== 'number')
      return this._update$3$bailout(2, posX, posY, t1, copyMap);
    if (!(posX >= t1))
      if (!(posY < 0)) {
        t1 = this._row;
        if (typeof t1 !== 'number')
          return this._update$3$bailout(3, posX, posY, copyMap, t1);
        t1 = posY >= t1;
      } else
        t1 = true;
    else
  ...
  t1 = this._col;
  if (typeof t1 !== 'number')
    return this._update$3$bailout(4, posX, posY, t1, copyMap);
  if (posX === t1 - 1) {
    t1 = this._row;
    if (typeof t1 !== 'number')
      return this._update$3$bailout(5, posX, posY, t1, copyMap);
    var t3 = posY >= t1 - 1;
    t1 = t3;
  } else
    t1 = false;
  if (t1) {
    t1 = this._route;
    if (typeof t1 !== 'number')
      return this._update$3$bailout(6, t1, 0, 0, 0);
    this._route = t1 + 1;
    return;
  }
  if (posY < 0 || posY >= copyMap.length)
    throw $.ioore(posY);
  t1 = copyMap[posY];
  if (typeof t1 !== 'string' && (typeof t1 !== 'object' || t1 === null || t1.constructor !== Array && !t1.is$JavaScriptIndexingBehavior()))
    return this._update$3$bailout(7, posX, posY, t1, copyMap);
  if (posX < 0 || posX >= t1.length)
    throw $.ioore(posX);
  if (t1[posX] === true)
    return;
  else {
    t1 = copyMap[posY];
    if (typeof t1 !== 'object' || t1 === null || (t1.constructor !== Array || !!t1.immutable$list) && !t1.is$JavaScriptIndexingBehavior())
      return this._update$3$bailout(8, posX, posY, t1, copyMap);
    if (posX < 0 || posX >= t1.length)
      throw $.ioore(posX);
    t1[posX] = true;
  }
  this._update$3(posX + 1, posY, copyMap);
  this._update$3(posX, posY + 1, copyMap);
  this._update$3(posX - 1, posY, copyMap);
  this._update$3(posX, posY - 1, copyMap);
},
 _update$3$bailout: function(state, env0, env1, env2, env3) {
  switch (state) {
    case 1:
      var posX = env0;
      var posY = env1;
  ...
  switch (state) {
    ...
    case 1:
      state = 0;
    default:
      if (state === 3 || state === 2 || state === 0 && !(posX < 0))
        switch (state) {
          case 0:
            var t1 = this._col;
          case 2:
            state = 0;
          case 3:
            if (state === 3 || state === 0 && !$.geB(posX, t1))
              switch (state) {
                case 0:
                case 3:
                  if (state === 3 || state === 0 && !(posY < 0))
                    switch (state) {
                      case 0:
                        t1 = this._row;
                      case 3:
    ...
    case 4:
      state = 0;
    case 5:
      if (state === 5 || state === 0 && posX === $.sub(t1, 1))
        switch (state) {
          case 0:
            t1 = this._row;
          case 5:
            state = 0;
            var t3 = $.geB(posY, $.sub(t1, 1));
            t1 = t3;
        }
      else
        t1 = false;
    case 6:
      if (state === 6 || state === 0 && t1)
        switch (state) {
          ...
          case 6:
            state = 0;
            this._route = $.add(t1, 1);
            return;
        }
      t1 = $.index(copyMap, posY);
    case 7:
      state = 0;
    case 8:
      if (state === 0 && $.index(t1, posX) === true)
        return;
      else
        switch (state) {
          case 0:
 ...
 OnesanRobot$2: function(_row, _col) {
  this._init$0();
}
};
...
$$.main_anon = {"":
 ...
 call$1: function(event$) {
  var rowTxt = $.query('#row');
  var colTxt = $.query('#col');
  $.OnesanRobot$($.add($.parseInt(rowTxt.get$value()), 1), $.add($.parseInt(colTxt.get$value()), 1)).start$0();
}
};
...
$.OnesanRobot$ = function(_row, _col) {
  var t1 = new $.OnesanRobot(_row, _col, null, null);
  t1.OnesanRobot$2(_row, _col);
  return t1;
};
...
$.main = function() {
  $.add$1($.query('#calcBtn').get$on().get$click(), new $.main_anon());
};
...
//@ sourceMappingURL=example06.dart.js.map